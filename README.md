...
- 正文里写的 `#关键词` 会在发布时被自动转换成小红书真正的可点击话题标签:`publisher.py` 会提取这些词、逐个用 `get_topic` 校验是否存在对应话题(校验不通过的会被丢弃,而不是硬发导致整条发布失败),再作为 `noteInfo.topics` 传给 `post_note`,由平台自己生成标签 ID 和展示文本——而不是把 `#关键词` 原样当成正文里的文字发出去。
- 小红书正文有 **1000 字硬上限**(算上最终追加的话题标签文字)。`synthesizer.py` 生成时会把目标字数控制在 800 字以内给标签留余量,超限会重试,重试还超就在句子/段落边界截断;`publisher.py` 发布前还会按最终会发送的真实字数做一次硬校验,超限直接跳过而不是拿去发个必失败的请求。命令行审核和网页审核卡片都会实时显示当前字数,超过 1000 会标红提醒。
